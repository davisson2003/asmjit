...
public:
  ...
  inline void init(const CpuFeatures& other) noexcept { std::memcpy(this, &other, sizeof(*this)); }
  ...
  inline BitWord* getBits() noexcept { return _bits; }
  //! Get all features as `BitWord` array (const).
  inline const BitWord* getBits() const noexcept { return _bits; }

  //! Get whether feature `feature` is present.
  inline bool has(uint32_t feature) const noexcept {
  ...
  inline bool hasAll(const CpuFeatures& other) const noexcept {
  ...
  //! Add a CPU `feature`.
  inline CpuFeatures& add(uint32_t feature) noexcept {
    ASMJIT_ASSERT(feature < kMaxFeatures);
  ...
  inline CpuFeatures& remove(uint32_t feature) noexcept {
    ASMJIT_ASSERT(feature < kMaxFeatures);
...
public:
  ...
  enum X86Features : uint32_t {
    ...
    kX86FeatureALTMOVCR8,                //!< CPU supports `MOV CR8` via `LOCK MOV CR0` in 32-bit mode (AMD CPUs).
    ...
    kX86FeatureMMX2,                     //!< CPU has MMX2 (Extended MMX).
    kX86Feature3DNOW,                    //!< CPU has 3DNOW.
    kX86Feature3DNOW2,                   //!< CPU has 3DNOW2 (Enhanced 3DNOW).
  ...
  inline CpuInfo(const CpuInfo& other) noexcept { init(other); }
  ...
  inline void initArch(uint32_t archType, uint32_t archMode = 0) noexcept {
    _archInfo.init(archType, archMode);
  }

  inline void init(const CpuInfo& other) noexcept { std::memcpy(this, &other, sizeof(*this)); }
  inline void reset() noexcept { std::memset(this, 0, sizeof(*this)); }
  ...
  inline uint32_t getArchSubType() const noexcept { return _archInfo.getSubType(); }
    ...
    //! Get CPU vendor ID.
  inline uint32_t getVendorId() const noexcept { return _vendorId; }
  //! Get CPU family ID.
  inline uint32_t getFamilyId() const noexcept { return _familyId; }
  //! Get CPU model ID.
  inline uint32_t getModelId() const noexcept { return _modelId; }
  //! Get CPU stepping.
  inline uint32_t getStepping() const noexcept { return _stepping; }

  //! Get number of hardware threads available.
  inline uint32_t getHwThreadsCount() const noexcept { return _hwThreadsCount; }

  //! Get all CPU features.
  inline const CpuFeatures& getFeatures() const noexcept { return _features; }
  //! Get whether CPU has a `feature`.
  inline bool hasFeature(uint32_t feature) const noexcept { return _features.has(feature); }
  //! Add a CPU `feature`.
  inline CpuInfo& addFeature(uint32_t feature) noexcept { _features.add(feature); return *this; }

  //! Get CPU vendor string.
  inline const char* getVendorString() const noexcept { return _vendorString; }
  //! Get CPU brand string.
  inline const char* getBrandString() const noexcept { return _brandString; }
  ...
  //! Get CPU type.
  inline uint32_t getX86ProcessorType() const noexcept { return _x86Data._processorType; }
  //! Get CPU brand index.
  inline uint32_t getX86BrandIndex() const noexcept { return _x86Data._brandIndex; }
  //! Get the size of a cache line flush.
  inline uint32_t getX86FlushCacheLineSize() const noexcept { return _x86Data._flushCacheLineSize; }
  //! Get the number of maximum logical processors.
  inline uint32_t getX86MaxLogicalProcessors() const noexcept { return _x86Data._maxLogicalProcessors; }
