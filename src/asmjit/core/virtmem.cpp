...
struct VirtMemManager::RBNode {
  ...
  static ASMJIT_FORCEINLINE bool isRed(RBNode* node) noexcept { return node && node->red; }
...
//!
//! Single rotation.
static ASMJIT_FORCEINLINE RBNode* rbRotateSingle(RBNode* root, int dir) noexcept {
...
//!
//! Double rotation.
static ASMJIT_FORCEINLINE RBNode* rbRotateDouble(RBNode* root, int dir) noexcept {
...
struct VirtMemManager::MemNode : public RBNode {
  ...
  // Get available space.
  ASMJIT_FORCEINLINE size_t getAvailable() const noexcept { return size - used; }
