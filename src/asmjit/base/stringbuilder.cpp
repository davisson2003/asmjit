...
ASMJIT_FAVOR_SIZE char* StringBuilder::prepare(uint32_t op, size_t len) noexcept {
  if (op == kStringOpSet) {
    ...
    if (_capacity < len) {
      if (len >= std::numeric_limits<size_t>::max() - sizeof(intptr_t) * 2)
        return nullptr;

      size_t to = IntUtils::alignUp<size_t>(len, sizeof(intptr_t));
    ...
    // Overflow.
    if (std::numeric_limits<size_t>::max() - sizeof(intptr_t) * 2 - _length < len)
      return nullptr;
    ...
    if (_capacity < after) {
      ...
      if (to < after) {
        ...
        if (to < (std::numeric_limits<size_t>::max() - 1024 * 32))
          to = IntUtils::alignUp<size_t>(to, 1024 * 32);
      }
...
Error StringBuilder::_opHex(uint32_t op, const void* data, size_t len) noexcept {
  ...
  if (len >= std::numeric_limits<size_t>::max() / 2 || !(dst = prepare(op, len * 2)))
    return DebugUtils::errored(kErrorNoHeapMemory);;
