...
namespace IntUtils {
  ...
  static inline int32_t floatAsInt(float f) noexcept { FloatBits m; m.f = f; return m.i; }
  //! Bit-cast 32-bit integer to `float`.
  static inline float intAsFloat(int32_t i) noexcept { FloatBits m; m.i = i; return m.f; }

  //! Bit-cast `double` to a 64-bit integer.
  static inline int64_t doubleAsInt(double d) noexcept { DoubleBits m; m.d = d; return m.i; }
  //! Bit-cast 64-bit integer to `double`.
  static inline double intAsDouble(int64_t i) noexcept { DoubleBits m; m.i = i; return m.d; }
  ...
  //! Map an integer `x` of type `T` to `int32_t` or `int64_t` depending on the
  //! type. Used internally by AsmJit to dispatch arguments that can be of arbitrary
  //! integer type into a function argument that is either `int32_t` or `int64_t`.
  template<typename T>
  static constexpr typename IntTraits<T>::IntType asInt(T x) noexcept {
    return static_cast<typename IntTraits<T>::IntType>(x);
  }

  template<typename T>
  static constexpr typename IntTraits<T>::UIntType asUInt(T x) noexcept {
  ...
  static constexpr uint32_t pack32_4x8(uint32_t b0, uint32_t b1, uint32_t b2, uint32_t b3) noexcept {
    return ASMJIT_PACK32_4x8(b0, b1, b2, b3);
  }

  //! Pack two 32-bit integer into a 64-bit integer as it is an array of `{u0,u1}`.
  static constexpr uint64_t pack64_2x32(uint32_t u0, uint32_t u1) noexcept {
    return ASMJIT_ARCH_LE ? (uint64_t(u1) << 32) + u0 : (uint64_t(u0) << 32) + u1;
  }
  ...
  //! \internal
  template<typename T>
  static constexpr T _blsiImpl(T x) noexcept { return x & (T(0) - x); }

  //! Extract lowest set isolated bit, like BLSI instruction, but portable.
  template<typename T>
  static constexpr T blsi(T x) noexcept { return T(_blsiImpl(asUInt(x))); }
  ...
  template<uint64_t N>
  static ASMJIT_INLINE uint32_t staticCtz() noexcept { return StaticCtz<N>::kValue; }
  ...
  template<typename T>
  static constexpr bool isBetween(T x, T a, T b) noexcept { return x >= a && x <= b; }
  ...
  static constexpr bool isInt4(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (isBetween<SignedType>(SignedType(x), -8, 7))
                                   : (UnsignedType(x) <= UnsignedType(7U));
  }
  ...
  static constexpr bool isInt8(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (sizeof(T) <= 1 || isBetween<SignedType>(SignedType(x), -128, 127))
                                   : (UnsignedType(x) <= UnsignedType(127U));
  }
  ...
  static constexpr bool isInt16(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (sizeof(T) <= 2 || isBetween<SignedType>(SignedType(x), -32768, 32767))
                                   : (sizeof(T) <= 1 || UnsignedType(x) <= UnsignedType(32767U));
  }
  ...
  static constexpr bool isInt32(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (sizeof(T) <= 4 || isBetween<SignedType>(SignedType(x), -2147483647 - 1, 2147483647))
                                   : (sizeof(T) <= 2 || UnsignedType(x) <= UnsignedType(2147483647U));
  }
  ...
  static constexpr bool isUInt4(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (x >= T(0) && x <= T(15))
                                   : (UnsignedType(x) <= UnsignedType(15U));
  }
  ...
  static constexpr bool isUInt8(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (x >= T(0) && (sizeof(T) <= 1 ? true : x <= T(255)))
                                   : (sizeof(T) <= 1 || UnsignedType(x) <= UnsignedType(255U));
  }
  ...
  static constexpr bool isUInt12(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (x >= T(0) && (sizeof(T) <= 1 ? true : x <= T(4095)))
                                   : (sizeof(T) <= 1 || UnsignedType(x) <= UnsignedType(4095U));
  }
  ...
  static constexpr bool isUInt16(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (x >= T(0) && (sizeof(T) <= 2 ? true : x <= T(65535)))
                                   : (sizeof(T) <= 2 || UnsignedType(x) <= UnsignedType(65535U));
  }
  ...
  static constexpr bool isUInt32(T x) noexcept {
    ...
    return IntTraits<T>::kIsSigned ? (x >= T(0) && (sizeof(T) <= 4 ? true : x <= T(4294967295U)))
                                   : (sizeof(T) <= 4 || UnsignedType(x) <= UnsignedType(4294967295U));
  }

  //! Get whether the `x` is a power of two (only one bit is set).
  template<typename T>
  static constexpr bool isPowerOf2(T x) noexcept { return x != 0 && (x & (x - 1)) == 0; }
  ...
  template<typename T>
  static ASMJIT_INLINE T alignUpPowerOf2(T base) noexcept {
...
# pragma warning(pop)
  ...
  template<typename X, typename Y>
  static constexpr X alignDiff(X base, Y alignment) noexcept {
    return alignUp(base, alignment) - base;
  }
  ...
  static constexpr Dst maskFromBool(Src b) noexcept {
    ...
    return Dst(UnsignedType(0) - UnsignedType(b));
  }
  ...
  static constexpr bool hasBit(T x, Index n) noexcept {
    return (x & (T(1) << n)) != 0;
  }
  ...
  public:
    enum : uint32_t { kBitWordSizeInBits = uint32_t(sizeof(BitWordT)) * 8 };
