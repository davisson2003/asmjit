...
// [Guard]
#ifndef _ASMJIT_BASE_MEMUTILS_H
#define _ASMJIT_BASE_MEMUTILS_H
...
static ASMJIT_INLINE uint32_t readU8(const void* p) noexcept { return uint32_t(static_cast<const uint8_t*>(p)[0]); }
static ASMJIT_INLINE int32_t readI8(const void* p) noexcept { return int32_t(static_cast<const int8_t*>(p)[0]); }
...
template<size_t ALIGNMENT>
static ASMJIT_INLINE uint32_t readU16x(const void* p) noexcept { return ASMJIT_ARCH_LE ? readU16xLE<ALIGNMENT>(p) : readU16xBE<ALIGNMENT>(p); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE int32_t readI16x(const void* p) noexcept { return ASMJIT_ARCH_LE ? readI16xLE<ALIGNMENT>(p) : readI16xBE<ALIGNMENT>(p); }
...
template<size_t ALIGNMENT>
static ASMJIT_INLINE int32_t readI32xLE(const void* p) noexcept { return int32_t(readU32xLE<ALIGNMENT>(p)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE int32_t readI32xBE(const void* p) noexcept { return int32_t(readU32xBE<ALIGNMENT>(p)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE uint32_t readU32x(const void* p) noexcept { return ASMJIT_ARCH_LE ? readU32xLE<ALIGNMENT>(p) : readU32xBE<ALIGNMENT>(p); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE int32_t readI32x(const void* p) noexcept { return ASMJIT_ARCH_LE ? readI32xLE<ALIGNMENT>(p) : readI32xBE<ALIGNMENT>(p); }
...
template<size_t ALIGNMENT>
static ASMJIT_INLINE int64_t readI64xLE(const void* p) noexcept { return int64_t(readU64xLE<ALIGNMENT>(p)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE int64_t readI64xBE(const void* p) noexcept { return int64_t(readU64xBE<ALIGNMENT>(p)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE uint64_t readU64x(const void* p) noexcept { return ASMJIT_ARCH_LE ? readU64xLE<ALIGNMENT>(p) : readU64xBE<ALIGNMENT>(p); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE int64_t readI64x(const void* p) noexcept { return ASMJIT_ARCH_LE ? readI64xLE<ALIGNMENT>(p) : readI64xBE<ALIGNMENT>(p); }
...
static ASMJIT_INLINE void writeU8(void* p, uint32_t x) noexcept { static_cast<uint8_t*>(p)[0] = uint8_t(x & 0xFFU); }
static ASMJIT_INLINE void writeI8(void* p, int32_t x) noexcept { static_cast<uint8_t*>(p)[0] = uint8_t(x & 0xFF); }
...
template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI16xLE(void* p, int32_t x) noexcept { writeU16xLE<ALIGNMENT>(p, uint32_t(x)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI16xBE(void* p, int32_t x) noexcept { writeU16xBE<ALIGNMENT>(p, uint32_t(x)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI16x(void* p, int32_t x) noexcept { writeU16x<ALIGNMENT>(p, uint32_t(x)); }
...
template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI32xLE(void* p, int32_t x) noexcept { writeU32xLE<ALIGNMENT>(p, uint32_t(x)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI32xBE(void* p, int32_t x) noexcept { writeU32xBE<ALIGNMENT>(p, uint32_t(x)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI32x(void* p, int32_t x) noexcept { writeU32x<ALIGNMENT>(p, uint32_t(x)); }
...
template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI64xLE(void* p, int64_t x) noexcept { writeU64xLE<ALIGNMENT>(p, uint64_t(x)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI64xBE(void* p, int64_t x) noexcept { writeU64xBE<ALIGNMENT>(p, uint64_t(x)); }

template<size_t ALIGNMENT>
static ASMJIT_INLINE void writeI64x(void* p, int64_t x) noexcept { writeU64x<ALIGNMENT>(p, uint64_t(x)); }
