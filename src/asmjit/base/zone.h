...
public:
  ...
  ASMJIT_INLINE void sort(const CMP& cmp = CMP()) noexcept {
...
public:
  explicit inline ZoneHash(ZoneAllocator* allocator = nullptr) noexcept : ZoneHashBase(allocator) {}
  ...
  inline Node* put(Node* node) noexcept { return static_cast<Node*>(_put(node)); }
  inline Node* del(Node* node) noexcept { return static_cast<Node*>(_del(node)); }
};
