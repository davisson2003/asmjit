...
public:
  ...
  ASMJIT_INLINE Error add(RAWorkReg* workReg, uint32_t flags, uint32_t allocable, uint32_t useId, uint32_t useRewriteMask, uint32_t outId, uint32_t outRewriteMask) noexcept {
    ...
    if (!tiedReg) {
      ...
      tiedReg->useRewriteMask |= useRewriteMask;
      tiedReg->outRewriteMask |= outRewriteMask;
      return kErrorOk;
    }
  }
...
public:
  ...
  ASMJIT_INLINE Error assignRAInst(CBNode* node, RABlock* block, RAInstBuilder& ib) noexcept {
