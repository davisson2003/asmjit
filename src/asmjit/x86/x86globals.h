...
// ============================================================================
// [asmjit::x86::CmpPredicate]
// ============================================================================

//! A predicate used by CMP[PD|PS|SD|SS] instructions.
enum CmpPredicate : uint32_t {
...
// ============================================================================
// [asmjit::x86::PCmpStrPredicate]
// ============================================================================

//! A predicate used by [V]PCMP[I|E]STR[I|M] instructions.
enum PCmpStrPredicate : uint32_t {
  // Source data format:
...
// ============================================================================
// [asmjit::x86::VPComPredicate]
// ============================================================================

//! A predicate used by VPCOM[U][B|W|D|Q] instructions (XOP).
enum VPComPredicate : uint32_t {
  kVPComLT                = 0x00U,       //!< Less.
...
// ============================================================================
// [asmjit::x86::RoundPredicate]
// ============================================================================

//! A predicate used by ROUND[PD|PS|SD|SS] instructions.
enum RoundPredicate : uint32_t {
...
//!   - `shufpd`
static constexpr uint32_t shufImm(uint32_t a, uint32_t b) noexcept {
  return (a << 1) | b;
}
...
//!   - `pshufhw()`
//!   - `shufps()`
static constexpr uint32_t shufImm(uint32_t a, uint32_t b, uint32_t c, uint32_t d) noexcept {
  return (a << 6) | (b << 4) | (c << 2) | d;
}

//! Create an immediate that can be used by VTERNLOG[D|Q] instructions.
static constexpr uint32_t tlogImm(uint32_t b000, uint32_t b001, uint32_t b010, uint32_t b011, uint32_t b100, uint32_t b101, uint32_t b110, uint32_t b111) noexcept {
  return (b000 << 0) | (b001 << 1) | (b010 << 2) | (b011 << 3) | (b100 << 4) | (b101 << 5) | (b110 << 6) | (b111 << 7);
}

//! Create an immediate that can be used by VTERNLOG[D|Q] instructions.
static constexpr uint32_t tlogVal(uint32_t x) noexcept { return x & 0xFF; }
//! Negate an immediate that can be used by VTERNLOG[D|Q] instructions.
static constexpr uint32_t tlogNot(uint32_t x) noexcept { return x ^ 0xFF; }
//! Create an if/else logic that can be used by VTERNLOG[D|Q] instructions.
static constexpr uint32_t tlogIf(uint32_t cond, uint32_t a, uint32_t b) noexcept { return (cond & a) | (tlogNot(cond) & b); }
